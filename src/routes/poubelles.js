...
// 🔹 Récupérer toutes les poubelles (admin + mobile)
router.get("/", (req, res) => {
...
router.put("/etat/:id", (req, res) => {
  ...
  connection.query(
    "UPDATE poubelles SET etat = ? WHERE id = ?",
    [etat, id],
    (err) => {
...
router.put("/bloquer/:id", (req, res) => {
  ...
  connection.query(
    "UPDATE poubelles SET bloquee = 1 WHERE id = ?",
    [id],
    (err) => {
...
router.put("/debloquer/:id", (req, res) => {
  ...
  connection.query(
    "UPDATE poubelles SET bloquee = 0 WHERE id = ?",
    [id],
    (err) => {
